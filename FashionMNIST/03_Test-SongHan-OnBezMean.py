...
model_state_path="model_state/03_bez_on_mean/[0.81, 0.81, 0.81, 0.71]/0.199/+/1699.pt"
if torch.cuda.is_available():
...
# prune_rate_range=[4.9,5.1,5.2,5.3,5.4,5.5,5.6]
# prune_rate_range=[5.1,6.1,7.1,8.1]
prune_rate_range=[4.2,4.3,4.4,4.5,4.6,4.7,4.8,5]
...
if os.path.isfile(os.path.join("results_sheet","03song_hn_onBezMeans.csv")):
    df_base=pd.read_csv(os.path.join("results_sheet","03song_hn_onBezMeans.csv"))
    df_concat=pd.concat([df_base,dataframe_results])
    dataframe_results=df_concat

dataframe_results.to_csv(os.path.join("results_sheet","03song_hn_onBezMeans.csv"),index=False)
