...
#         print(m_2[0])        
        ...
        print("m1",torch.count_nonzero(m_1),"m2",torch.count_nonzero(m_2),"m_new",torch.count_nonzero(m_new))        
#         print(m_new[0])        
...
#                     print("from",start_index,"to",start_index+each_bin_count)
                    ...
                    proportion_to_prune=min(proportion[bin_index],torch.tensor(1))
#                     if proportion_to_prune>0.999:
# #                         print("approxing")
#                         proportion_to_prune=1
#                     proportion_to_prune=round(proportion_to_prune.item())
...
#                     if proportion_to_prune<1:
#                         print("proportion to prune =",proportion_to_prune)
#                         print(len(selected_indices_prune),"out of ",len(elem_indices),"to be pruned")
                    
                    
#                     if len(selected_indices_prune)!=len(elem_indices):
#                         print(len(selected_indices_prune),"out of ",len(elem_indices),"to be pruned")
...
def fit(epochs, lr, model, train_loader, val_loader, opt_func=torch.optim.SGD,
    ...
    for epoch in range(epochs):
        ...
        if mask_whole_model and range_params and upscale:
            ...
            print("mask comressions=",get_mask_compression(mask_whole_model),get_mask_compression(new_mask_list))
...
# times=    [500,  50, 10,  40,   40, 40, 50,    50,   60,  60]
# upsc_vals=[ 0.01,0.05,0.1 ,0.2, 0.3, 0.5, 0.7, 0.75, 0.8, 0.9]

# times=    [3,        3,         3,    ]
# upsc_vals=[0.9,     0.99,  0.9999]


times=    [5,        ]
upsc_vals=[0.99999999999999999,]


times=    [20,        ]
upsc_vals=[0.9999999999999999,]
...
for upscale_method in upscale_method_list:
    for range_params in range_params_list:
        ...
        list_mask_whole_model=prune_model_get_mask_bezier(model,range_params,all_processes[0],upscale_method)  
        apply_mask_model(model,list_mask_whole_model)
        total_size,nz_size=model_size(model)
        compression=(total_size-nz_size)/total_size
        res = evaluate(model, test_loader)
        print("1st, Compression=",compression,"Result is ",res)


        
        
        i=0
        for upscale in all_processes[1:]:



            
            print(range_params,upscale,upscale_method,i,"/",len(all_processes))
            ...
            compression=(total_size-nz_size)/total_size
            res = evaluate(model, test_loader)
            print(i,"Compression=",compression,"Result after pruning is ",res,"Going for re training")
...
if os.path.isfile(os.path.join("results_sheet","controlled04bezier_on_actl_Wts17Jan2021.csv")):
    df_base=pd.read_csv(os.path.join("results_sheet","controlled04bezier_on_actl_Wts17Jan2021.csv"))
    df_concat=pd.concat([df_base,df])
    df=df_concat

df.to_csv(os.path.join("results_sheet","controlled04bezier_on_actl_Wts17Jan2021.csv"),index=False)