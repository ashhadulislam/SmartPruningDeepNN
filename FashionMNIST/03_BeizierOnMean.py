...
for upscale_method in upscale_method_list:
    ...
    #             print("Compression=",compression,"Result after pruning is ",res,"Going for re training")
                ...
                history_prune,_,list_mask_whole_model = fit(epochs, lr, model, train_loader, val_loader,
                                  mask_whole_model=list_mask_whole_model, range_params=range_params,
...
#file already exists
if os.path.isfile(os.path.join("results_sheet","03_bezier_on_mean_18JanTry.csv")):
    df_base=pd.read_csv(os.path.join("results_sheet","03_bezier_on_mean_18JanTry.csv"))
    df_concat=pd.concat([df_base,df])
    df=df_concat

df.to_csv(os.path.join("results_sheet","03_bezier_on_mean_18JanTry.csv"),index=False)        
