...
def fit(epochs, lr, model, train_loader, val_loader, opt_func=torch.optim.SGD,
    ...
    for epoch in range(epochs):
        ...
        if mask_whole_model and range_params and upscale and upscale_method:
            # regenerate mask
            new_mask_list=prune_mean_weights_get_mask_bezier(model,range_params,upscale,
...
#         print("Numnon 0s",torch.count_nonzero(m_1),torch.count_nonzero(m_2),torch.count_nonzero(m_new))
#         print(m_new[0])        
...
#                     print("proportion ",proportion_to_prune,"and count to prune",count_to_prune)
...
for upscale_method in upscale_method_list:
    for range_params in range_params_list:
        ...
        for upscale in upscale_list:
            ...
            list_mask_whole_model=prune_mean_weights_get_mask_bezier(model,
                                                                     range_params,
                                                                     upscale,mean_weight_description_weighted,upscale_method)
    ...
    #             print("Compression=",compression,"Result after pruning is ",res,"Going for re training")
                ...
                pruned_model_state_path="model_state/03_bezier_on_mean_weights.pt"
                history_prune,_,list_mask_whole_model = fit(epochs, lr, model, train_loader, val_loader,
                                 ...
                                 upscale=upscale,mean_weight_description_weighted=mean_weight_description_weighted,
...
if os.path.isfile(os.path.join("results_sheet","03_bezier_on_mean_03JanTry.csv")):
    df_base=pd.read_csv(os.path.join("results_sheet","03_bezier_on_mean_03JanTry.csv"))
    df_concat=pd.concat([df_base,df])
    df=df_concat

df.to_csv(os.path.join("results_sheet","03_bezier_on_mean_03JanTry.csv"),index=False)        
